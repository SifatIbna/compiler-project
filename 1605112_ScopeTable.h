...
public:
    ...
    SymbolInfo *lookup(string target)
    {
        ...
        while(temp!=0)
        {
            if(temp->get_symbol_name()==target)
            {
                return temp;
            }
            temp=temp->get_next();
    ...
    bool Delete(string name)
    {
        ...
        next=temp;
        int pos=0;
        while(next->get_symbol_name()!=name)
        {
            prev=next;
            next=next->get_next();
            pos++;
    ...
    void printScopeTable()
    {
        ...
        while (temp)
        {
	        fprintf(logout,"< %s : %s > ",temp->get_type().c_str(),temp->get_symbol_name().c_str());


            // cout << "<" << temp->get_symbol_name() << " : " << temp->get_type()<< ">  ";
            temp = temp->get_next();
        }
	    fprintf(logout,"\n");
